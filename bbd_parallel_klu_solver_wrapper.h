#ifndef _BBD_PARALLEL_KLU_SOLVER_H_
#define _BBD_PARALLEL_KLU_SOLVER_H_
...
void bbd_parallel_klu_solve(SparseMatrix *A_org, SparseMatrix *Ch_matrix, SparseMatrix *BU_matrix,
                            std::vector<int_bbd> &node_index, std::vector<float_bbd> &time_step, Params *params)
{
  ...
  for (int i = 0; i < A_org->get_nrow(); i++)
    klu_rhs[i] = 1.0;
  ...
  //     printf("x[%d] = %f \n", i, klu_rhs[i]);
  double klu_time;
  tmp_timer.elapsedWallclockTime(klu_time);
  // if (comm_rank == 0)
  // {
  ...
  for (int i = 0; i < A_order_metis->get_nrow(); i++)
    P[Q[i]] = i;
  ...
  int numTOcalc = n_parts / comm_size;
  int start_point_parts = comm_rank * numTOcalc;
  std::vector<SparseMatrix *> matrix_arr_single(numTOcalc, NULL);
  std::vector<float_bbd *> xi_arr_single(numTOcalc, NULL);    // used to store each rhs, xi will be written to this arrary if it's sequential BBD
  std::vector<BBDSchur *> solver_arr(numTOcalc, NULL);        // solver array for each partition
  std::vector<SparseMatrix *> schur_mtx_arr(comm_size, NULL); // CA-1B
  SparseMatrix *schur_mtx_arr_single;
  float_bbd *schur_rhs_arr_single;

  float_bbd *tmp_rhs_before, *tmp_rhs, *xc, *x_sol, *final_sol;
  ...
  for (int i = 0; i < nrow; i++)
    tmp_rhs[i] = tmp_rhs_before[metis_perm[Q[i]]];
  ...
  for (int i = 0; i < numTOcalc; i++)
  {
    matrix_arr_single[i] = extract_one_partition(A_metis, start_point, n_parts, start_point_parts + i); // check done
    xi_arr_single[i] = extract_one_rhs(tmp_rhs, start_point, start_point_parts + i);                    // check done  xi
  }
  if (comm_rank == 0)
    D_matrix = get_Dc_matrix(matrix_arr_single[0]); // matrix in right-bottom corner; check done

  // unroll this for loop using MPI
  for (int i = 0; i < numTOcalc; i++)
  {
    // printf("factorize partition %d,  size: %d, schur_size: %d, nnz, %d, diag_count: %d\n", start_point_parts + i,
    //        matrix_arr_single[i]->get_nrow(), matrix_arr_single[i]->get_schur_size(), matrix_arr_single[i]->get_nnz(), matrix_arr_single[i]->get_diag_count());
    solver_arr[i] = new BBDSchur(SOLVER_TYPE_BBD_KLU, matrix_arr_single[i]->get_schur_size(), matrix_arr_single[i]);
    solver_arr[i]->fact();
    if (i == 0)
    {
      schur_mtx_arr_single = solver_arr[i]->get_Schur();                     // CA-1B
      schur_rhs_arr_single = solver_arr[i]->get_Schur_RHS(xi_arr_single[i]); // CA-1bi
    }
    else
    {
      schur_mtx_arr_single = bbd_mm_add(schur_mtx_arr_single, solver_arr[i]->get_Schur(), 1.0, 1.0, CSPARSE_BLAS, true);
      schur_rhs_arr_single = bbd_dense_add(schur_size, schur_rhs_arr_single, solver_arr[i]->get_Schur_RHS(xi_arr_single[i]), 1.0, MKL_BLAS);
    }
  }

  gather_schur_mtx(schur_mtx_arr, schur_mtx_arr_single, comm_rank, comm_size);
  float_bbd *schur_rhs_sum = new float_bbd[schur_size](); // initialize all element 0
  gather_schur_rhs(schur_rhs_sum, schur_rhs_arr_single, schur_size);

  klu_symbolic *Symbolic2;
  klu_numeric *Numeric2;
  klu_common Common2;
  if (comm_rank == 0)
  {
    for (int i = 0; i < comm_size; i++)
    {
      // printf("add partition from rank %d \n", i);
      D_matrix = bbd_mm_add(D_matrix, schur_mtx_arr[i], 1.0, -1.0, CSPARSE_BLAS, true); // the last param used to free memory of old Dc_mtx
    }
    bbd_dense_add(schur_size, schur_rhs_sum, xc, -1.0, MKL_BLAS); // in-place operation  y = ax + y
    // printf("GMRES Solve \n");
    // printf("D_matrix size: %d, nnz, %d, density: %lf\n",
    //        D_matrix->get_nrow(), D_matrix->get_nnz(), D_matrix->get_density());
    klu_defaults(&Common2);
  ...
  MPI_Bcast(xc, schur_size, MPI_DOUBLE, root, MPI_COMM_WORLD);
  //  Ai * xc = bi - Bi * xc
  for (int i = 0; i < numTOcalc; i++)
  { // MPI_Bcast, broadcast xc all processes
    // printf("update and solve rhs partition %d \n", start_point_parts + i);
    solver_arr[i]->update_rhs(xi_arr_single[i], xc);          // in-place operation, will be written to             Ai xi  =  bi - Bi * xc
    solver_arr[i]->solve(xi_arr_single[i], xi_arr_single[i]); // in-place operation, Ly = b, Ux = y
  }

  int sendnum = start_point[start_point_parts + numTOcalc] - start_point[start_point_parts];
  float_bbd *xi_arr_tosend = new float_bbd[sendnum];
  for (int i = 0; i < numTOcalc; i++)
  {
    memcpy(&xi_arr_tosend[start_point[start_point_parts + i] - start_point[start_point_parts]],
           xi_arr_single[i], (start_point[start_point_parts + i + 1] - start_point[start_point_parts + i]) * sizeof(float_bbd));
  }

  x_sol = new float_bbd[start_point[n_parts + 1]];

  int *rhs_recvCount, *displs; // xi size of every part
  if (comm_rank == 0)
  {
    rhs_recvCount = new int[comm_size];
    for (int i = 0; i < comm_size; i++)
    {
      rhs_recvCount[i] = start_point[i * numTOcalc + numTOcalc] - start_point[i * numTOcalc];
    }
    displs = new int[comm_size];
    displs[0] = 0;
    for (int i = 0; i < comm_size - 1; i++)
    {
      displs[i + 1] = displs[i] + rhs_recvCount[i];
    }
  }

  gather_xi(x_sol, xi_arr_tosend, rhs_recvCount, displs, sendnum);

  if (comm_rank == 0)
  {
    // printf("Merge solution \n");
    memcpy(&x_sol[start_point[n_parts]], xc, schur_size * sizeof(float_bbd));

    for (int i = 0; i < nrow; i++)
      final_sol[i] = x_sol[P[metis_iperm[i]]];
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  {
    final_sol = update_rhs(ts, BU_matrix, Ch_matrix, final_sol, params->tran_method);
    for (int i = 0; i < nrow; i++)
      tmp_rhs[i] = final_sol[metis_perm[Q[i]]];

    float_bbd *klu_rhs = new float_bbd[A_org->get_nrow()]();
    for (int i = 0; i < A_org->get_nrow(); i++)
      klu_rhs[i] = final_sol[i];
    // Common.btf = 0; Common.ordering=2; Common.scale=0;
    klu_solve(Symbolic, Numeric, A_org->get_nrow(), 1, klu_rhs, &Common);

    MPI_Bcast(tmp_rhs, nrow, MPI_DOUBLE, root, MPI_COMM_WORLD);
